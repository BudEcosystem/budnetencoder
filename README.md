...
1. Enforcing orthogonal, inexpensive channel mixing
2. Robust scaling against activation outliers  
3. Judicious mixed precision on the most sensitive paths
4. Stable normalization under small effective batch sizes
...
Let $x \in \mathbb{R}^{B \times S \times d}$ be the residual stream. A ternary linear maps $xW^{\top}$, where $W \in \{-1, 0, 1\}^{d_{\text{out}} \times d_{\text{in}}}$ with per-output scale $s_i > 0$. A $b$-bit symmetric uniform quantizer is:

$$\mathcal{Q}_b(u; \Delta) = \Delta \cdot \text{clip}\left(\text{round}\left(\frac{u}{\Delta}\right), -L, L\right)$$
...
**Orthogonal mixers.** Block-Hadamard: $H = \bigoplus_{k=1}^{K} H_b$ with $b \in \{32, 64, 128\}$, $Kb = d$. DPD: $O = D_2 P D_1$, where $D_i$ are diagonal $\pm 1$ matrices and $P$ a permutation. Both satisfy $O^{\top} O = I$.
...
$$\hat{w}_{ij} = s_i \cdot \text{sign}(w_{ij} - t_i) \cdot \mathbf{1}(|w_{ij} - t_i| > \delta_i)$$

We learn $(s_i, t_i, \delta_i)$ with:

• **Positive scale:** $s_i = \text{softplus}(\tilde{s}_i)$

• **Thresholds:** $t_i$ initialized from the 30th percentile of $|w|$; $\delta_i$ small (e.g., $0.05 \cdot \text{median}|w|$) and learned

• **LSQ gradient scaling:** for scale parameters,

$$\frac{\partial \mathcal{L}}{\partial s_i} \gets \frac{1}{\sqrt{N_i \cdot L}} \sum_j \left(\frac{\partial \mathcal{L}}{\partial \hat{w}_{ij}} \cdot q_{ij}\right), \quad q_{ij} \in \{-1, 0, 1\}$$
...
$$\text{QDyT}(x) = \gamma \odot \tanh\big(\alpha \cdot \text{clip}(x - \mu, a)\big) + \beta$$

• **Group-wise $\mu$:** mean over small channel groups (e.g., 8–16) per token

• **Learned parameters:** $\alpha, \gamma, \beta, a$ (with $a = \text{softplus}(\tilde{a})$)

• **Warm-up:** linearly ramp $\alpha$ from $0.05 \to 0.5$ over the first 2k steps
...
$$\mathbb{E}\left[(X - \mathcal{Q}_b(X))^2\right] \le \underbrace{\frac{\Delta^2}{12}}_{\text{in-range}} + \underbrace{\mathbb{E}[(|X| - \rho_p)^2 \mathbf{1}(|X| > \rho_p)]}_{\text{overload}}$$

and the second term can be driven down by EMA and moderate $p$ (e.g., 99.5–99.9).

### 4.3 Bounded Residual Networks with QDyT

With $\phi(x) = \gamma\tanh(\alpha \cdot \text{clip}(x - \mu, a)) + \beta$ and $|\phi'(x)| \le |\gamma|\alpha$, each residual block $x \mapsto x + \zeta f(\phi(x))$ has Lipschitz constant $\le 1 + \zeta L_f |\gamma|\alpha$. With $\zeta \approx O(1/\sqrt{L})$ and small $|\gamma|\alpha$ early in training (warm-up), depth-wise amplification is controlled, yielding stable forward/backward dynamics even with STE.
...
• **Optimizer:** AdamW ($\beta_1 = 0.9$, $\beta_2 = 0.98$, $\varepsilon = 10^{-8}$), cosine LR with 10k warm-up
...
• **Teacher guidance (optional):** KL on MLM logits from an FP16 teacher or EMA of the model before ternarization ($\tau = 1.5$–2.0)
...
• **Ablations:**
  - (a) DPD only vs Block-H+DPD vs dense H
  - (b) absmax vs percentile
  - (c) int4 gate vs 8-bit gate
  - (d) mixed-precision attention on/off
  - (e) TTQ/LSQ+ vs fixed ternary thresholds
  - (f) with/without QDyT calibration
  - (g) expand 6× vs 8×
...
• **LRs.** Base LR $3 \times 10^{-4}$ (ternary), $1.5 \times 10^{-4}$ (8-bit components), weight decay 0.01

• **Warm-up.** 10k steps; $\alpha$ ramp $0.05 \to 0.5$; kd_coeff cosine $0 \to 0.5$
